...
1. **Feature Extraction (Raw to Tensor):** Process raw audio into a **1,094-dimensional vector**.
...
| Property | Value |
|----------|-------|
| **Input Shape** | `[1, 1094]` (Float32) |
| **Output Shape** | `[1, 1094]` (Float32) |
...
|-------|-------------|--------|
| `rf_model.onnx` | `[1, 1094]` | Single float $P$ ∈ [0, 1] |
| `cnn_model.onnx` | `[1, 1094, 1]` | Single float $P$ ∈ [0, 1] |
| `cnnlstm_model.onnx` | `[1, 1094, 1]` | Single float $P$ ∈ [0, 1] |
| `tcn_model.onnx` | `[1, 1094, 1]` | Single float $P$ ∈ [0, 1] |
| `tssd_model.onnx` | `[1, 1094, 1]` | Single float $P$ ∈ [0, 1] |

**Note:** CNN, CNN-LSTM, TCN, and TSSD require an added "channel" dimension. Reshape the scaled `[1, 1094]` vector to `[1, 1094, 1]` before inference.
...
The 1,094-element vector must be assembled in this exact order:
...
|-------|-----------|-----------|------------|------|
| 1 | Wav2Vec2 + Librosa | Audio File | 1094 | Converts sound to math. |
| 2 | `scaler.onnx` | [1, 1094] | [1, 1094] | Standardizes values. |
| 3 | Base Ensemble | [1, 1094, 1] | 1 (each) | Captures diverse artifacts. |
